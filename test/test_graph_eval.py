...
class Test_graph_eval(unittest.TestCase):
    ...
    def test_distillDataframe(self):
        ...
        function_return, label, load = eval_graph.distillDataframe(
            formated_bible, False, 1, False
        )
        # function_return.sort_values(by=['character_A'])

        base = pd.read_csv("test/csv_test/test_return_distillBible.csv")
        base.drop(["Unnamed: 0"], axis=1, inplace=True)
    ...
    def test_dataframe2graph(self):
        ...
        edges, color_emotion, label = eval_graph.dataframe2graph(
            function_return, True, "character_A", "character_B", "emotion"
        )
    ...
    def test_adopt_clusters(self):
        ...
        res_cluster = pd.read_csv("test/csv_test/test_cluster_return.csv")
    ...
    def test_concat_cluster(self):
        df_cluster = pd.read_csv("test/csv_test/test_return_concat_clusters.csv")
        ...
        df_res = pd.read_csv("test/csv_test/test_return_concat_res.csv")
