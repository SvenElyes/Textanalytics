...
class TestUserFunctions(TestCase):
    ...
    def test_extract_characters_without_ruler(self):
        df_characters = character_extractor.extract_characters(
            df_test, rule_based_matching=False
        )
        df_wrong_param_1 = character_extractor.extract_characters(
            df_test, use_bible_patterns=False
        )
        df_wrong_param_2 = character_extractor.extract_characters(
            df_test, rule_based_matching=False
        )
        df_wrong_param_3 = character_extractor.extract_characters(
            df_test,
            rule_based_matching=False,
            patterns=[{"label": "PERSON", "pattern": "God"}],
        )
    ...
    def test_extract_characters_own_patterns(self):
        df_characters = character_extractor.extract_characters(
            df_test,
            use_bible_patterns=False,
            patterns=[{"label": "PERSON", "pattern": "earth"}],
        )
